...
-- Splitting into mob/web tables to avoid potential conflicts if web and mobile models run simultaneously
...
-- Single metadata table for web and model
...
CREATE TABLE {{.scratch_schema}}.{{.model}}_base_metadata_this_run{{.entropy}} (
  id VARCHAR(64),
  run_id TIMESTAMP,
  model_version VARCHAR(64),
  model VARCHAR(64),
...
INSERT INTO {{.output_schema}}.{{.model}}_base_event_id_manifest{{.entropy}} (
  ...
  WHERE
    (SELECT collector_tstamp FROM {{.output_schema}}.{{.model}}_base_event_id_manifest{{.entropy}} LIMIT 1) IS NULL
...
INSERT INTO {{.output_schema}}.{{.model}}_base_session_id_manifest{{.entropy}} (
  ...
  WHERE
    (SELECT min_tstamp FROM {{.output_schema}}.{{.model}}_base_session_id_manifest{{.entropy}} LIMIT 1) IS NULL
    -- ensures that the seed is not re-inserted if the table is populated.
);

-- Create staged tables:
{{if eq .model "web"}} 
CREATE TABLE IF NOT EXISTS {{.scratch_schema}}.{{.model}}_events_staged{{.entropy}} (
...
CREATE TABLE IF NOT EXISTS {{.scratch_schema}}.{{.model}}_events_staged{{.entropy}} (
  ...
  event_version VARCHAR(128) ENCODE ZSTD,
  event_fingerprint VARCHAR(128) ENCODE ZSTD,
  event_schema VARCHAR(2256) ENCODE ZSTD,
  true_tstamp TIMESTAMP ENCODE ZSTD,
