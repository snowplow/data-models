...
CREATE TABLE {{.scratch_schema}}.{{.model}}_base_run_metadata_temp{{.entropy}} AS (
  SELECT
    'run' AS id,
...
UPDATE {{.scratch_schema}}.{{.model}}_base_metadata_this_run{{.entropy}}
  ...
  FROM {{.scratch_schema}}.{{.model}}_base_run_metadata_temp{{.entropy}} b
  WHERE {{.scratch_schema}}.{{.model}}_base_metadata_this_run{{.entropy}}.id = b.id;

DROP TABLE IF EXISTS {{.scratch_schema}}.{{.model}}_base_run_dupe_metadata_temp{{.entropy}};


CREATE TABLE {{.scratch_schema}}.{{.model}}_base_run_dupe_metadata_temp{{.entropy}} AS (
  SELECT
    'run' AS id,
    SUM(num_rows) AS duplicate_rows_removed,
    count(DISTINCT event_id) AS distinct_keys_removed
  FROM {{.scratch_schema}}.{{.model}}_base_duplicates_this_run{{.entropy}}
);
...
UPDATE {{.scratch_schema}}.{{.model}}_base_metadata_this_run{{.entropy}}
  ...
  FROM {{.scratch_schema}}.{{.model}}_base_run_dupe_metadata_temp{{.entropy}} b
  WHERE {{.scratch_schema}}.{{.model}}_base_metadata_this_run{{.entropy}}.id = b.id;
