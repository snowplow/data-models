...
*/
--TODO: Create stored procedure to share with page_views
DROP TABLE IF EXISTS {{.scratch_schema}}.mobile_sv_run_metadata_temp{{.entropy}};

CREATE TABLE {{.scratch_schema}}.mobile_sv_run_metadata_temp{{.entropy}} AS (
  SELECT
    'run' AS id,
...
UPDATE {{.scratch_schema}}.mobile_sv_metadata_this_run{{.entropy}}
  ...
  FROM {{.scratch_schema}}.mobile_sv_run_metadata_temp{{.entropy}} b
  WHERE {{.scratch_schema}}.mobile_sv_metadata_this_run{{.entropy}}.id = b.id;
