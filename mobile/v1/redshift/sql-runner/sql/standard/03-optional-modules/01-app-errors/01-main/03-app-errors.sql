...
{{if eq (or .enabled false) true}}
  ...
  AS (
    ...
    INNER JOIN
      {{.scratch_schema}}.mobile_app_errors_events{{.entropy}} ae
      ON es.event_id = ae.root_id
      AND es.collector_tstamp = ae.root_tstamp

    WHERE
      es.event_name = 'application_error'
      AND es.collector_tstamp >= (SELECT lower_limit FROM {{.scratch_schema}}.mobile_app_errors_run_limits{{.entropy}})
      AND es.collector_tstamp <= (SELECT upper_limit FROM {{.scratch_schema}}.mobile_app_errors_run_limits{{.entropy}})
