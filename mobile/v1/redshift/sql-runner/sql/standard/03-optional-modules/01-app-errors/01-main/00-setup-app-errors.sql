...
{{if eq (or .enabled false) true}}
  ...
  CREATE TABLE {{.scratch_schema}}.mobile_app_errors_metadata_this_run{{.entropy}} (
    id VARCHAR(64),
    run_id TIMESTAMP,
    model_version VARCHAR(64),
    model VARCHAR(64),
...
{{if eq (or .enabled false) true}}
  {{if ne (or .skip_derived false) true}}
    ...
    CREATE TABLE IF NOT EXISTS {{.output_schema}}.mobile_app_errors{{.entropy}} (LIKE {{.scratch_schema}}.mobile_app_errors_staged{{.entropy}});
  {{end}}
{{end}}
