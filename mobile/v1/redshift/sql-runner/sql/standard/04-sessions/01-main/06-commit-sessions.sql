...
-- Create upsert limit
DROP TABLE IF EXISTS {{.scratch_schema}}.mobile_sessions_upsert_limit{{.entropy}};

CREATE TABLE {{.scratch_schema}}.mobile_sessions_upsert_limit{{.entropy}} AS (
  SELECT
...
BEGIN;
  ...
  {{if ne (or .skip_derived false) true}}
    ...
    DELETE FROM {{.output_schema}}.mobile_sessions{{.entropy}}
      WHERE session_id IN (SELECT session_id FROM {{.scratch_schema}}.mobile_sessions_this_run{{.entropy}})
      AND start_tstamp >= (SELECT lower_limit FROM {{.scratch_schema}}.mobile_sessions_upsert_limit{{.entropy}});

    INSERT INTO {{.output_schema}}.mobile_sessions{{.entropy}}
      (SELECT * FROM {{.scratch_schema}}.mobile_sessions_this_run{{.entropy}});
  {{end}}
  ...
  INSERT INTO {{.output_schema}}.datamodel_metadata{{.entropy}} (
    ...
    FROM {{.scratch_schema}}.mobile_sessions_metadata_this_run{{.entropy}}
  );
