...
CREATE TABLE {{.scratch_schema}}.mobile_sessions_run_metadata_temp{{.entropy}} AS (
  SELECT
    'run' AS id,
...
UPDATE {{.scratch_schema}}.mobile_sessions_metadata_this_run{{.entropy}}
  ...
  FROM {{.scratch_schema}}.mobile_sessions_run_metadata_temp{{.entropy}} b
  WHERE {{.scratch_schema}}.mobile_sessions_metadata_this_run{{.entropy}}.id = b.id;
