...
SET (LOWER_LIMIT, UPPER_LIMIT) = (SELECT AS STRUCT lower_limit, upper_limit FROM {{.scratch_schema}}.mobile_users_limits{{.entropy}});
...
AS(
  SELECT
    a.*
  FROM {{.output_schema}}.mobile_sessions{{.entropy}} a
  INNER JOIN {{.scratch_schema}}.mobile_users_userids_this_run{{.entropy}} b
