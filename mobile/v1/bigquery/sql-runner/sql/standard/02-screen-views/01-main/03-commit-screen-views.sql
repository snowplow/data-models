...
BEGIN
  ...
  {{if ne (or .skip_derived false) true}}
    ...
    CALL {{.output_schema}}.commit_table('{{.scratch_schema}}',                      -- sourceDataset
                                         'mobile_screen_views_this_run{{.entropy}}', -- sourceTable
                                         '{{.output_schema}}',                       -- targetDataset
                                         'mobile_screen_views{{.entropy}}',          -- targetTable
                                         'screen_view_id',                           -- joinKey
  ...
  {{if eq .stage_next true}}
    ...
    CALL {{.output_schema}}.commit_table('{{.scratch_schema}}',                      -- sourceDataset
                                         ...
                                         'mobile_screen_views_staged{{.entropy}}',   -- targetTable
                                         'screen_view_id',                           -- joinKey
                                         'derived_tstamp',                           -- partitionKey
                                         FALSE);                                     -- automigrate
