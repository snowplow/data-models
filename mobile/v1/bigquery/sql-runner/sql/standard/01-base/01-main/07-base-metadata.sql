...
CREATE OR REPLACE TABLE {{.scratch_schema}}.{{.model}}_base_run_metadata_temp{{.entropy}} AS (
  SELECT
    'run' AS id,
...
UPDATE {{.scratch_schema}}.{{.model}}_base_metadata_this_run{{.entropy}} a
  SET
    run_end_tstamp = CURRENT_TIMESTAMP(),
