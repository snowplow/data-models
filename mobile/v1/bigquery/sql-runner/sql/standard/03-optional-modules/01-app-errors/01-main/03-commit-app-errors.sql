  ...
  BEGIN
    ...
    {{if ne (or .skip_derived false) true}}
      ...
      CALL {{.output_schema}}.commit_table('{{.scratch_schema}}',                    -- sourceDataset
                                           'mobile_app_errors_this_run{{.entropy}}', -- sourceTable
                                           '{{.output_schema}}',                     -- targetDataset
                                           'mobile_app_errors{{.entropy}}',          -- targetTable
                                           'event_id',                               -- joinKey
                                           'derived_tstamp',                         -- partitionKey
                                           FALSE);                                   -- automigrate
    ...
    {{if eq .stage_next true}}
      ...
      CALL {{.output_schema}}.commit_table('{{.scratch_schema}}',                    -- sourceDataset
                                           ...
                                           'mobile_app_errors_staged{{.entropy}}',   -- targetTable
                                           'event_id',                               -- joinKey
                                           'derived_tstamp',                         -- partitionKey
                                           FALSE);                                   -- automigrate
