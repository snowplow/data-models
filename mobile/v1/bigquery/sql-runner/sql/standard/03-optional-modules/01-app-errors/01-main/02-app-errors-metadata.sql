...
{{if eq (or .enabled false) true}}
  ...
  CREATE OR REPLACE TABLE {{.scratch_schema}}.mobile_app_errors_run_metadata_temp{{.entropy}} AS (
    SELECT
      'run' AS id,
  ...
  UPDATE {{.scratch_schema}}.mobile_app_errors_metadata_this_run{{.entropy}} a
    SET
    ...
    FROM {{.scratch_schema}}.mobile_app_errors_run_metadata_temp{{.entropy}} b
    WHERE a.id = b.id;
