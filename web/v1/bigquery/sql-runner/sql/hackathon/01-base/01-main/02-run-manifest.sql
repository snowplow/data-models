...
CREATE OR REPLACE PROCEDURE {{.scratch_schema}}.run_manifest ()
...
BEGIN
  ...
  CALL {{.scratch_schema}}.log_model_table('{{.scratch_schema}}.base_run_manifest{{.entropy}}', 'debug', 'base');
  CALL {{.scratch_schema}}.log_model_table('{{.scratch_schema}}.base_session_id_run_manifest{{.entropy}}', 'debug', 'base');
END;
