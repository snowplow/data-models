...
CREATE OR REPLACE PROCEDURE {{.scratch_schema}}.commit_base_manifest ()
...
CREATE OR REPLACE PROCEDURE {{.scratch_schema}}.base_complete (level STRING, ends_run BOOL)
OPTIONS(strict_mode=false)
BEGIN
  CALL {{.scratch_schema}}.commit_base_manifest();
  CALL {{.scratch_schema}}.cleanup_model_tables(level, 'base', ends_run);
END;

-- TODO: Move this
-- CALL {{.scratch_schema}}.base_complete('debug');
