...
CREATE OR REPLACE PROCEDURE {{.scratch_schema}}.sessions_to_include (days_late_allowed INT64)
...
BEGIN
  ...
  AS(
    ...
    WHERE
      ...
      AND TIMESTAMP_DIFF(a.max_tstamp, COALESCE(b.min_tstamp, a.max_tstamp), DAY) <= days_late_allowed
      -- Compares the max_tstamp of new data to the min_tstamp for its existing session, if one exists.
  );
  CALL {{.scratch_schema}}.log_model_table('{{.scratch_schema}}.base_sessions_to_include{{.entropy}}', 'trace', 'base');
END;
