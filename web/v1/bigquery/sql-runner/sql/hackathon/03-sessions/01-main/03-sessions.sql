...
CREATE OR REPLACE PROCEDURE {{.scratch_schema}}.sessions_this_run ()
OPTIONS(strict_mode=false)
BEGIN
  CREATE OR REPLACE TABLE {{.scratch_schema}}.sessions_this_run{{.entropy}}
  CLUSTER BY domain_userid,domain_sessionid
  AS (
    SELECT
      -- app ID
      a.app_id,
  ...
  CALL {{.scratch_schema}}.log_model_table('{{.scratch_schema}}.sessions_this_run{{.entropy}}', 'debug', 'sessions');
END;
