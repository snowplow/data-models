...
CREATE OR REPLACE PROCEDURE {{.scratch_schema}}.page_view_events ()
...
BEGIN
  ...
  AS(
    ...
    -- TODO: Make this configurable via parameters instead of golang template?
    -- Note: Yet to find a way to do this easily... So it remains hard set by the templater for now.
  ...
  CALL {{.scratch_schema}}.log_model_table('{{.scratch_schema}}.pv_page_view_events{{.entropy}}', 'debug', 'page_views');
END;
