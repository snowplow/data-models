CREATE OR REPLACE TABLE {{.scratch_schema}}.sessions_this_run{{.entropy}}
AS (
  SELECT
    ...
    b.page_views,
    b.engaged_time_in_s,
    TIMESTAMP_DIFF(b.end_tstamp, b.start_tstamp, second)  AS absolute_time_in_s,
...
CREATE OR REPLACE TABLE {{.scratch_schema}}.sessions_staged{{.entropy}} AS(SELECT * FROM {{.scratch_schema}}.sessions_this_run{{.entropy}});

CREATE OR REPLACE TABLE {{.output_schema}}.sessions{{.entropy}} AS(SELECT * FROM {{.scratch_schema}}.sessions_this_run{{.entropy}});