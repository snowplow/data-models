...
AS (
  ...
  FROM
    {{.scratch_schema}}.users_aggregates{{.entropy}} AS b

  INNER JOIN {{.scratch_schema}}.users_sessions_this_run{{.entropy}} AS a
    ON a.domain_userid = b.domain_userid AND a.start_tstamp = b.start_tstamp

  INNER JOIN {{.scratch_schema}}.users_lasts{{.entropy}} AS c
