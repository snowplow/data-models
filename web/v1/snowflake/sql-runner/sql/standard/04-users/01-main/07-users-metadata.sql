...
CREATE OR REPLACE TABLE {{.scratch_schema}}.users_run_metadata_temp{{.entropy}}
AS (
  SELECT
    'run' AS id,
...
UPDATE {{.scratch_schema}}.users_metadata_this_run{{.entropy}}
  ...
  FROM
    {{.scratch_schema}}.users_run_metadata_temp{{.entropy}} AS b
  WHERE
    {{.scratch_schema}}.users_metadata_this_run{{.entropy}}.id = b.id;
