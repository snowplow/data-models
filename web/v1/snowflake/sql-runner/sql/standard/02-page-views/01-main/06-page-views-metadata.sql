...
CREATE OR REPLACE TABLE {{.scratch_schema}}.pv_run_metadata_temp{{.entropy}}
AS (
  SELECT
    'run' AS id,
...
UPDATE {{.scratch_schema}}.pv_metadata_this_run{{.entropy}}
  ...
  FROM
    {{.scratch_schema}}.pv_run_metadata_temp{{.entropy}} AS b
  WHERE
    {{.scratch_schema}}.pv_metadata_this_run{{.entropy}}.id = b.id;

UPDATE {{.scratch_schema}}.pv_metadata_this_run{{.entropy}}
  SET
    duplicate_rows_removed = b.duplicate_rows_removed
  FROM
    {{.scratch_schema}}.pv_run_dupe_metadata_temp{{.entropy}} AS b
  WHERE
    {{.scratch_schema}}.pv_metadata_this_run{{.entropy}}.id = b.id;
