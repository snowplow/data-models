...
CALL {{.output_schema}}.commit_staged(
  'FALSE',                                                                    -- automigrate
  UPPER('{{.scratch_schema}}.sessions_userid_manifest_this_run{{.entropy}}'), -- stage source
  UPPER('{{.scratch_schema}}.sessions_userid_manifest_staged{{.entropy}}'),   -- stage target
  UPPER('domain_userid')                                                      -- joinKey
);
  ...
  CALL {{.output_schema}}.mk_transaction(
    ...
    DELETE FROM {{.output_schema}}.sessions{{.entropy}}
      WHERE domain_sessionid IN (SELECT domain_sessionid FROM {{.scratch_schema}}.sessions_this_run{{.entropy}})
        AND start_tstamp >= (SELECT TIMEADD(DAY, -{{or .upsert_lookback_days 30}}, MIN(start_tstamp)) FROM {{.scratch_schema}}.sessions_this_run{{.entropy}});
...
INSERT INTO {{.output_schema}}.datamodel_metadata{{.entropy}}
  ...
  FROM
    {{.scratch_schema}}.sessions_metadata_this_run{{.entropy}};
