...
AS (
  ...
  FROM
    {{.scratch_schema}}.page_views_staged{{.entropy}} AS a

  INNER JOIN {{.scratch_schema}}.sessions_aggregates{{.entropy}} AS b
    ON a.domain_sessionid = b.domain_sessionid AND a.page_view_in_session_index = b.page_views
);
